...
// Original enum from https://github.com/meiskam/PlayerHeads
...
public enum TexturedSkullType {
    ...
    private final SkullType bukkitSkull;

    TexturedSkullType(SkullType bukkitSkull, String ownerUUID, String texture) {
        this.owner = UUID.fromString(ownerUUID);
        this.texture = texture;
        this.bukkitSkull = bukkitSkull;
    }

    TexturedSkullType(String ownerUUID, String texture) {
        this(SkullType.PLAYER, ownerUUID, texture);
    }

    public ItemStack createHead(String displayName) {
        ItemStack head = new ItemStack(Material.SKULL_ITEM, 1, (short) this.bukkitSkull.ordinal());

        if (this.bukkitSkull != SkullType.PLAYER) {
            return head;
        }

        SkullMeta meta = (SkullMeta) head.getItemMeta();

        PlayerProfile profile = Bukkit.createProfile(this.owner);
    ...
    public static TexturedSkullType getByEntityType(EntityType type) {
        return TexturedSkullType.valueOf(type.name());
    }
}
